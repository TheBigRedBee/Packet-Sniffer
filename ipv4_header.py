...
class IPv4Header(LayerHeader):
    def __init__(self, pkt):
        ...
        header = unpack("!BBHHHBBHII", self.header_bytes)
        ...
        self.TTL = header[5]
        self.transport_protocol = header[6]
        self.checksum = header[7]
        self.source_addr = header[8]
        self.dest_addr = header[9]

        if int(self.IHL) > 5:
            option_size = (self.IHL * 4) - header_length
            header = unpack("!BBHHHBBHII" + str(option_size) + 's', pkt)
            self.options_bytes = header[10]
        else:
            self.options_bytes = None
