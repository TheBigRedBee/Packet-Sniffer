...
class TCPHeader(LayerHeader):
    def __init__(self, pkt):
        ...
        header = unpack("!HHIIBBHHH", self.header_bytes)
        ...
        if self.data_offset > 5:
            ...
            header = unpack("!HHIIBBHHH" + str(option_size) + 's', pkt)
            self.options_bytes = header[9]
        else:
            self.options_bytes = None
        
        
        self.NS_flag = header[4] & 0x01
        self.CWR_flag = (header[5] & 0x80) >> 7
        self.ECE_flag = (header[5] & 0x40) >> 6
        self.URG_flag = (header[5] & 0x20) >> 5
        self.ACK_flag = (header[5] & 0x10) >> 4
        self.PSH_flag = (header[5] & 0x08) >> 3
        self.RST_flag = (header[5] & 0x04) >> 2
        self.SYN_flag = (header[5] & 0x02) >> 1
        self.FIN_flag = header[5] & 0x01
        self.window_size = header[6]
        self.checksum = header[7]
        self.urg_pointer = header[8]
